...
function exportSystem(){
  ...
  // System meta — stamp the editor credit into the description
  const _exportIS = Object.assign({}, systemSettings.importSettings);
  const _editorTag = 'Edited by SFS System Editor Site';
  if(!(_exportIS.description||'').includes(_editorTag)){
    _exportIS.description = (_exportIS.description && _exportIS.description !== 'n/a')
      ? _exportIS.description + '\n' + _editorTag
      : _editorTag;
  }
  zipFiles[`${sysName}/Import_Settings.txt`]   = enc(JSON.stringify(_exportIS, null, 2));
