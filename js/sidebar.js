...
function _liveSyncNow(){
  ...
  if(tog('ter-has'))
  {
    ...
    // Bust terrain sample + surface caches so verticeSize, rock settings and
    // other TERRAIN_DATA fields (not just the formula) take effect immediately.
    if(typeof invalidateTerrainCache === 'function') invalidateTerrainCache(selectedBody);
...
document.getElementById('sidebar').addEventListener('input',  e => {
  // hm-raw-view is edited as a block and saved explicitly via hmSaveRaw();
  // letting it fire liveSync on every keystroke would read stale tf-normal data.
...
function hmRefreshLoadedList(){
  ...
  if(!document.getElementById('hm-list-filter-wrap')){
    const wrap = document.createElement('div');
    wrap.id = 'hm-list-filter-wrap';
    wrap.style.cssText = 'margin-bottom:5px;position:relative';
    wrap.innerHTML = `
      <input id="hm-filter" placeholder="Search heightmaps…" oninput="hmApplyFilter()"
        style="width:100%;font-size:.62rem;padding:5px 10px;border-radius:4px;background:var(--dp3,var(--bg1));border:1px solid var(--ac20,var(--ink5));color:var(--ink2);font-family:'JetBrains Mono',monospace;box-sizing:border-box;outline:none">`;
    list.parentNode.insertBefore(wrap, list);
  }

  // Cards
  window._hmAllCards = hms.map((e, i) => {
    const base = e.name.replace(/\.[^.]+$/, '');
    const ext  = e.name.match(/\.[^.]+$/)?.[0] || '';
    const isImg = /\.(png|jpe?g)/i.test(e.name);
    const preview = isImg && e.url
      ? `<img src="${e.url}" style="height:28px;max-width:60px;object-fit:cover;border-radius:2px;border:1px solid var(--ink5);image-rendering:pixelated">`
      : `<span style="font-size:.55rem;color:var(--ink4);font-family:'JetBrains Mono',monospace;background:var(--bg1);padding:2px 5px;border-radius:3px;border:1px solid var(--ink5)">${ext.replace('.','').toUpperCase()}</span>`;
    return { base, html: `<div data-hm="${base}" style="display:flex;align-items:center;gap:6px;padding:5px 6px;background:var(--bg2);border-radius:5px;margin-bottom:3px">
      ${preview}
      <span style="flex:1;font-size:.62rem;font-family:'JetBrains Mono',monospace;color:var(--ink2);overflow:hidden;text-overflow:ellipsis;white-space:nowrap" title="${e.name}">${base}</span>
      <button onclick="hmInsertMap('${base}')" style="font-size:.55rem;padding:2px 7px;border-radius:3px;background:var(--sky2);color:#000;border:none;cursor:pointer;font-family:'JetBrains Mono',monospace;font-weight:700;flex-shrink:0">INSERT</button>
    </div>` };
  });
  ...
  // Quick-insert chips
  insertRow.innerHTML = hms.map(e => {
    const base = e.name.replace(/\.[^.]+$/, '');
    return `<button onclick="hmInsertMap('${base}')" style="font-size:.55rem;padding:2px 8px;border-radius:10px;background:var(--bg3);color:var(--sky2);border:1px solid var(--sky2);cursor:pointer;font-family:'JetBrains Mono',monospace">${base}</button>`;
  }).join('');
}

function hmApplyFilter(){
  const list = document.getElementById('hm-loaded-list');
  if(!list || !window._hmAllCards) return;
  const q = (document.getElementById('hm-filter')?.value || '').toLowerCase();
  list.innerHTML = window._hmAllCards
    .filter(c => !q || c.base.toLowerCase().includes(q))
    .map(c => c.html).join('');
}

function hmToggleCollapse(){ /* no-op — collapse removed */ }
...
function _hmSyncRawView(){
  ...
  rv.readOnly = true;
  document.getElementById('hm-save-raw-btn').style.display = 'none';
}
...
function hmEditRaw(){
  ...
  if(!rv) return;
  rv.readOnly = false;
  rv.style.color = 'var(--ink1)';
  rv.style.borderColor = 'var(--sky2)';
  document.getElementById('hm-save-raw-btn').style.display = '';
  rv.focus();
}
