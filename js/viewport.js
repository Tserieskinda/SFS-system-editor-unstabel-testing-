...
function _drawViewportNow(){
  ...
  names.forEach(name => {
    ...
    // Terrain heightmap can extend beyond physR_px (tall bumps on small bodies like
    // asteroids). Use the cached N=360 terrain sample to get the true max terrain
    // radius, so the body is not culled when zoomed in and the centre is off-screen
    // but the heightmap silhouette is still visible. Fall back to a ×1.5 overestimate
    // while the sample is not yet cached — safe, cheap, one-time per body.
    if(b.data.TERRAIN_DATA && physR_px > 0){
      const _cullSamp = _getTerrainSamples(name, b, bodyRadius_m * radiusMult, 360, null);
      if(_cullSamp){
        const R_m = bodyRadius_m * radiusMult;
        let _terrMaxFrac = 0;
        for(let _ci = 0; _ci < _cullSamp.heights.length; _ci++){
          const f = _cullSamp.heights[_ci] / R_m;
          if(f > _terrMaxFrac) _terrMaxFrac = f;
        }
        _cullR = Math.max(_cullR, physR_px * (1 + _terrMaxFrac));
      } else {
        // Sample not yet ready — use a conservative ×1.5 overestimate to avoid
        // premature culling on the first few frames while the cache warms up.
        _cullR = Math.max(_cullR, physR_px * 1.5);
      }
    }
